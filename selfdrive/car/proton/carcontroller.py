...
def apply_proton_steer_torque_limits(apply_torque, apply_torque_last, driver_torque, LIMITS):
  ...
  driver_max_torque = LIMITS.STEER_MAX + driver_torque * 8
  driver_min_torque = -LIMITS.STEER_MAX + driver_torque * 8
...
class CarControllerParams():
  def __init__(self, CP):
    ...
    self.STEER_DELTA_UP = 14                      # torque increase per refresh, 0.8s to max
    self.STEER_DELTA_DOWN = 22                    # torque decrease per refresh
